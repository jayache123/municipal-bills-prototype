...
| **Anthropic prompt caching** | System prompt is long and re-sent in full on every extraction call. Caching it server-side would cut input token costs by ~90% and shave a few seconds per bill. Low effort, worth doing before production. | Before production deploy |
| **Model tiering: Haiku → Sonnet fallback** | Most clean digital bills likely extract correctly on Haiku (~5× cheaper, faster). Only escalate to Sonnet when confidence falls below `confidence_threshold_review`. Aligns with the two-pass design already in the settings table. | Before production deploy; requires accuracy testing on Haiku against the full test corpus |
| **Async / queue-based extraction** | Current pipeline is synchronous — the user waits 60–90 s per bill. For bulk volumes (hundreds or thousands of bills), extraction must run in the background. Architecture: upload → enqueue job → worker extracts → DB updated → UI polls or receives push notification. Vercel Queues is the natural fit on this stack. | Before onboarding any client processing more than ~20 bills at a time |
| **Parallel extraction for bulk ingestion** | Running extractions sequentially caps throughput at ~40 bills/hour. Parallelising (e.g. 10–20 concurrent API calls, rate-limited to stay within Anthropic's TPM limits) would bring that to 400–800 bills/hour. Depends on async architecture above. | Same as async queue item |
| **Anthropic Batch API for overnight runs** | Batch API offers 50% cost reduction and is designed for large, non-real-time workloads (up to 10,000 requests per batch, processed within 24 hours). Ideal for month-end bulk ingestion where results are not needed immediately. | When a client needs to process a full month's bill corpus in one run |
